"""
This is the python implementation of policy iteration algorithm
...
def getUtil(reward, V, gamma, T, action, state):
    reward = reward[state]
    u = np.multiply(T[state, :, int(action)], V).sum()
    return reward + gamma * u
...
def policyIteration(gamma, theta, r, silent = False):
    start = time.time()
    ...
    while(1): #Do until convergence
        ...
        for state in range(n_states):
            action = policy[state]
            v1[state] = getUtil(rewards, v1, gamma, T, action, state)
